...
type Bit = 0 | 1;
type WordLength = 1 | 2 | 4 | 8 | 16 | 32 | 64 | 128 | 256 | 512 | 1024;
type Word = Bit[];

const WORD_LENGTH: WordLength = 8; // specifies how many bits are in a word

// 8-bit architecture, word example: [0,    0,  0,  0, 0, 0, 0, 0]
//                     values:       [128, 64, 32, 16, 8, 4, 2, 1]
type WordFactoryOpts = { length?: WordLength, bitVal?: Bit };
function wordFactory(opts?: WordFactoryOpts): Word {
...
function positiveIntegerToWord(integerToMap: number): Word {
    // 83 / 2 = 41, remainder 1
    // 41 / 2 = 20, remainder 1
    // 20 / 2 = 10, remainder 0
    // 10 / 2 = 5,  remainder 0
    ...
    return wordFactory().map((_, index) => {
        ...
        const bit: Bit = integerToMap >= bitValue ? 1 : 0;
        integerToMap -= bit * bitValue;
        return bit;
    });
}

function wordToPositiveInteger(word: Word): number {
    return word.reduce((accumulator, currentValue, index) => {
        const bitValue = 2 ** (WORD_LENGTH - index - 1);
        return accumulator + currentValue * bitValue as Bit;
    }, 0);
}

const computer = {
    add(word1: Word, word2: Word): Word {
        return wordFactory().map((_, index) => {
    ...
    subtract(word1: Word, word2: Word): Word {
        return wordFactory().map((_, index) => {
            const bit1 = word1[index];
            const bit2 = word2[index];
    ...
    multiply(word1: Word, word2: Word): Word {
        return wordFactory().map((_, index) => {
            const bit1 = word1[index];
            const bit2 = word2[index];
            return bit1 * bit2 >= 1 ? 1 : 0;
    ...
    divide(word1: Word, word2: Word): Word {
        return wordFactory().map((_, index) => {
            const bit1 = word1[index];
            const bit2 = word2[index];
            return bit1 / bit2 >= 1 ? 1 : 0;
    ...
    compare(word1: Word, word2: Word): Word {
        return wordFactory().map((_, index) => {
            const bit1 = word1[index];
            const bit2 = word2[index];
    ...
    and(word1: Word, word2: Word): Word {
        return wordFactory().map((_, index) => {
            const bit1 = word1[index];
            const bit2 = word2[index];
            return bit1 && bit2 ? 1 : 0;
    ...
    xor(word1: Word, word2: Word): Word {
        return wordFactory().map((_, index) => {
            const bit1 = word1[index];
            const bit2 = word2[index];
            return bit1 !== bit2 ? 1 : 0;
